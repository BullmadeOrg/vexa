...
Vexa requests `verbose_json` for plain model ids so compatible backends can return timestamps and
confidence metadata. Provider-qualified GPT model ids such as `openai/gpt-transcribe` or
`x-ai/grok-stt-1.0` use portable text-only `json`. OpenRouter's `openai/whisper-1` route is the
timestamp-capable exception: Vexa requests `verbose_json` plus `timestamp_granularities[]=word` and
preserves its top-level word timings for live agreement and speaker boundaries.

For OpenRouter with real word timestamps:

```bash
TRANSCRIPTION_SERVICE_URL=https://openrouter.ai/api
TRANSCRIPTION_SERVICE_TOKEN=<openrouter-key>
