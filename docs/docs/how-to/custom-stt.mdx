...
confidence metadata. Provider-qualified model ids such as `openai/gpt-transcribe` or
`x-ai/grok-stt-1.0` automatically use portable `json`, which works with OpenRouter's text-only STT
routes.
