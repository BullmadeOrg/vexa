...
TRANSCRIPTION_RESPONSE_FORMAT=json  # for providers that reject verbose_json
```
...
Vexa requests `verbose_json` by default so compatible backends can return timestamps and confidence
metadata. Set `TRANSCRIPTION_RESPONSE_FORMAT=json` for text-only routes such as OpenRouter's Grok STT
and GPT Transcribe models.
